    ...
    pcall(function()
        ...
        if playerGui then
            for _, gui in pairs(playerGui:GetDescendants()) do
                if gui:IsA("GuiObject") and gui.Name ~= "NoLagGui" and not gui:IsDescendantOf(qocPlayer.PlayerGui:FindFirstChild("NoLagGui")) then
...
startButton.MouseButton1Click:Connect(function()
    qocStartNoLagFunction()  -- ИСПРАВЛЕНО: вызов функции вместо присваивания
end)
...
stopButton.MouseButton1Click:Connect(function()
    qocStopNoLagFunction()  -- ИСПРАВЛЕНО: вызов функции вместо присваивания
end)

print("[NoLag] GUI загружен! Используйте кнопки Start/Stop")
